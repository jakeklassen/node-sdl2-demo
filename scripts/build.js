...
const includes = ["src/**/*.js", "assets/**/*", "package.json"];
...
try {
	echo(`Packaging ${name}@${version} for ${platform}-${arch} (node ${nodeVersion})`);
	...
	if (!res.ok) {
		throw new Error(`Failed to download Node.js runtime: ${res.status} ${res.statusText} (${url})`);
	}
	...
	echo(`  Run it with:  ./${path.relative(projectPath, path.join(finalDir, name))}`);
} finally {
